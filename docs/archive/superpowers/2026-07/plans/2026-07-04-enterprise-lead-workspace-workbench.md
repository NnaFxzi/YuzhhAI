...
**Goal:** Replace the workspace home default with a single-screen management workbench for Agents, skills, research
capabilities, and domestic content platforms.

**Architecture:** Keep execution/run behavior in the existing console files, but introduce a separate renderer workbench
component for the landing page. Add pure UI metadata helpers so tests can verify navigation, Agent management metadata,
and configuration sections without mounting Electron UI.
