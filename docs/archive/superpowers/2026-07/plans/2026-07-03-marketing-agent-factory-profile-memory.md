...
**Goal:** Refine `推广agent` so its default behavior is a single-factory, memory-backed promotion assistant that
extracts stable factory facts, reuses them, and distinguishes temporary campaign details from long-term profile changes.

**Architecture:** This first implementation is prompt-level and preset-level only. `src/main/presetAgents.ts` owns the
agent identity and prompt, while `src/main/agentManager.ts` auto-installs and refreshes the managed preset so existing
users receive the updated prompt without changing runtime fields such as model, cwd, pinning, or enabled state.
...
Mirror the behavior in `systemPromptEn`, including one-factory profile, temporary task details, long-term update
signals, conflict handling, and one-sentence context confirmation.
