...
`lobsterai-server` no longer accepts `public` for HTML share creation or update. HTML shares now support share-code
access only.
...
The `accessMode` form field is now optional. If omitted, the server defaults to `code`. If the client sends
`accessMode=public`, the server returns an error.
...
- Existing legacy shares with `accessMode: "public"` may still appear from lookup/list APIs; updating them should use
  code mode so the server returns a new share code.
