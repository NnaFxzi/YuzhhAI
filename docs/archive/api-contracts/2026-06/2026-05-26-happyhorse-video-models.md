...
lobsterai-server adds Aliyun Bailian HappyHorse video-generation support through the existing media endpoints. New model
IDs are:
...
Pricing is pre-deducted from subscription/free/reward/boost credits before the upstream task is created. Long duration
requests are rejected before submission when the user does not have enough credits.
...
Generation, polling, list, and cancel task endpoints require Electron JWT bearer auth, except model listing which
remains public.
...
- Generated upstream video URLs are temporary; clients should download or persist them promptly if long retention is
  needed.
