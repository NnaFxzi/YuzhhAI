...
**Architecture:** Add explicit cloud capability helpers, make endpoint helpers return disabled/local URLs by default, and gate cloud-only callers before fetch. Keep compatibility identifiers that are runtime protocols.
...
- [ ] Rename public action prefix constants to neutral names while keeping event strings stable only if tests require it.
...
- [ ] Replace Youdao/LobsterAI default endpoint URLs with disabled empty strings or `https://www.yuzhh.com` informational URLs.
...
- [ ] Run `npm run build`.
- [ ] Run final source scan for `youdao.com`, `api-overmind`, `rlogs`, and old portal URLs; document intentional remaining compatibility identifiers.
