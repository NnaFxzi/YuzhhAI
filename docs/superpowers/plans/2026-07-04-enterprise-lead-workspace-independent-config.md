...
**Goal:** Make each enterprise lead workspace own its model provider, skill, external research, and domestic platform research configuration.

**Architecture:** Store the workspace-specific configuration inside `EnterpriseLeadWorkspace.settings` with explicit `model`, `skillIds`, `externalResearch`, and `domesticResearch` sections. The renderer edits those sections directly on the workbench page, while the main process normalizes and persists them. Enterprise lead model generation resolves API credentials from the workspace's own provider settings instead of only the global app config.
...
- [ ] Normalize legacy `modelRef`, `skillCapabilities`, `researchCapabilities`, and `contentPlatforms` so existing rows still load.
