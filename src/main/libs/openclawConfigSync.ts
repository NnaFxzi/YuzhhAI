...
function deriveNimAccountId(
  instance: Pick<NimInstanceConfig, 'nimToken' | 'appKey' | 'account'>,
): string | null {
...
const MANAGED_SKILL_ENTRY_OVERRIDES: Record<string, { enabled: boolean }> = {
  ...
  // The app configures MCP servers via openclaw.json mcp.servers field.
  ...
  // users are routed through the app's MCP layer instead.
  mcporter: {
    enabled: false,
  },
};
...
const MANAGED_WEB_SEARCH_POLICY_PROMPT = [
  ...
  `- Only use the ${APP_DISPLAY_NAME} \`web-search\` skill when local command execution is available. Native channel sessions may deny \`exec\`, so prefer \`browser\` or \`web_fetch\` there.`,
  ...
  '',
  `Do not claim you searched the web unless you actually used \`browser\`, \`web_fetch\`, or the ${APP_DISPLAY_NAME} \`web-search\` skill.`,
].join('\n');
...
const MANAGED_BROWSER_POLICY_PROMPT = [
  '## Browser Policy',
  '',
  `${APP_DISPLAY_NAME} does not support sandbox browser execution in this version.`,
  '- For every `browser` tool call, set `target="host"` explicitly.',
  `- Do not use \`target="sandbox"\` or \`target="node"\` unless a future ${APP_DISPLAY_NAME} version explicitly enables it.`,
 ...
 *
 * Example outputs:
 *   macOS:   ~/Library/Application Support/宇智汇和 AI 助手/SKILLs
 *   Windows: ~/AppData/Roaming/宇智汇和 AI 助手/SKILLs
 *   Linux:   ~/.config/宇智汇和 AI 助手/SKILLs
 */
...
const buildManagedSkillCreationPrompt = (skillsDirPath: string): string =>
  [
    '## Skill Creation',
    '',
    `When the user asks you to create a new skill, you MUST place it under the ${APP_DISPLAY_NAME} skills directory:`,
    '',
...
const clampModelMaxTokens = (
  rawMaxTokens: number | undefined,
...
const cloneAgentModelDefault = (entry: OpenClawAgentModelDefault): OpenClawAgentModelDefault =>
...
const stripExplicitContextCacheProviderSuffix = (modelId: string, provider?: string): string => {
  ...
  return normalized.endsWith(suffix) ? modelId.slice(0, modelId.length - suffix.length) : modelId;
};
...
const shouldApplyProviderExplicitContextCacheDefault = (providerName?: string): boolean => {
  ...
  return (
    normalizedProvider === ProviderName.Anthropic ||
...
const pluginMatches = (plugin: PreinstalledOpenClawPlugin, ...ids: string[]): boolean =>
  ids.includes(plugin.packageId) || ids.includes(plugin.pluginId);
...
function buildOpenClawMcpServers(
  ...
  for (const server of servers) {
    ...
    if (server.transportType !== 'stdio') {
      ...
      if (!normalizedUrl.ok) {
        console.warn(
          `[OpenClawConfigSync] skipped MCP server "${server.name}" because its URL is invalid`,
        );
        continue;
      }
...
export class OpenClawConfigSync {
  ...
  private readonly getBrowserWebAccessConfig: () =>
    Partial<BrowserWebAccessConfig> | null | undefined;
  ...
  sync(reason: string): OpenClawConfigSyncResult {
    ...
    const agentModelDefaults =
      Object.keys(perModelCustomDefaults).length > 0
    ...
    const mainAgentWorkingDirectory =
      agents.find(agent => agent.id === AgentId.Main)?.workingDirectory?.trim() || '';
    const taskWorkingDirectory =
      mainAgentWorkingDirectory || (runtimeCoworkConfig.workingDirectory || '').trim();
    ...
    console.log(
      `${gwDiagTs()} existingGateway keys:`,
      Object.keys(existingGateway).sort().join(',') || '(empty)',
    );
    console.log(
      `${gwDiagTs()} existingPlugins keys:`,
      Object.keys(existingPlugins).sort().join(',') || '(empty)',
    );
    console.log(
      `${gwDiagTs()} existingPluginEntries keys:`,
      Object.keys(existingPluginEntries).sort().join(',') || '(empty)',
    );
    ...
    const managedConfig: Record<string, unknown> = {
        ...
        defaults: {
          ...
          ...(runtimeCoworkConfig.embeddingEnabled
            ? {
                memorySearch: {
                  enabled: true,
                  provider: ['openai', 'gemini', 'voyage', 'mistral', 'ollama'].includes(
                  ...
                  remote: {
                    ...(coworkConfig.embeddingRemoteBaseUrl
                      ? { baseUrl: coworkConfig.embeddingRemoteBaseUrl }
                      : {}),
                    ...(coworkConfig.embeddingRemoteApiKey
                      ? { apiKey: coworkConfig.embeddingRemoteApiKey }
                      : {}),
                  },
                  store: {
      ...
      ...(() => {
        ...
        const transientPluginIds = [...(hasPreinstalledPlugin('openclaw-lark') ? ['feishu'] : [])];
        const cleanedExistingEntries = Object.fromEntries(
          Object.entries(existingPluginEntries).filter(
            ([id]) => !knownStalePluginIds.includes(id) && !transientPluginIds.includes(id),
          ),
        );
        ...
        const pluginEntries: Record<string, unknown> = {
            ...
            preinstalledPlugins.map(plugin => {
              ...
              const pluginEnabled = (() => {
                if (pluginMatches(plugin, DINGTALK_OPENCLAW_CHANNEL, 'dingtalk'))
                  return dingTalkInstances.some(i => i.enabled && i.clientId);
                ...
                if (pluginMatches(plugin, 'moltbot-popo'))
                  return popoInstances.some(i => i.enabled && i.appKey);
          ...
          ...(hasIndustryPositioningPlugin
            ? { 'lobster-industry-positioning': { enabled: true } }
            : {}),
          ...
          // Disable acpx (ACP agent runtime) - the app does not use ACP and
        ...
        const existingAllow = Array.isArray((existingPlugins as Record<string, unknown>).allow)
          ? ((existingPlugins as Record<string, unknown>).allow as unknown[]).filter(
              (id): id is string => typeof id === 'string' && id.length > 0,
            )
          : [];
        const trustedPluginAllow = Array.from(
          new Set([
            ...existingAllow,
        ...
        return Object.keys(pluginEntries).length > 0
          ? {
              plugins: {
                ...
                ...(() => {
                  const paths = [findBundledExtensionsDir(), findThirdPartyExtensionsDir()].filter(
    ...
    if (managedConfig.plugins) {
      ...
      const existingMemoryCoreConfig = (existingMemoryCore as Record<string, unknown>).config as
    ...
    if (enabledDingTalkInstances.length > 0) {
      const buildDingTalkAccountConfig = (
        ...
        // gatewayBaseUrl were app-specific and are not in the plugin schema.
        dmPolicy: inst.dmPolicy || 'open',
        allowFrom: (() => {
          const ids = inst.allowFrom?.length ? [...inst.allowFrom] : [];
          if (inst.dmPolicy === 'open' && !ids.includes('*')) ids.push('*');
    ...
    if (configuredNimInstances.length > 0) {
      ...
      configuredNimInstances.forEach((inst, idx) => {
        ...
        const preferredKey =
          deriveNimAccountConfigKey(inst) || deriveNimAccountId(inst) || `nim_${idx + 1}`;
        const accountKey = accounts[preferredKey]
          ? deriveNimAccountId(inst) || `${preferredKey}_${idx + 1}`
          : preferredKey;
    ...
    if (configChanged) {
      ...
      try {
        ...
        if (curGw !== nxtGw) {
          ...
          console.log(
            `${gwDiagTs()} old gateway keys:`,
            Object.keys(currentObj.gateway ?? {})
              .sort()
              .join(','),
          );
          console.log(
            `${gwDiagTs()} new gateway keys:`,
            Object.keys(nextObj.gateway ?? {})
              .sort()
              .join(','),
          );
        ...
        if (curPl !== nxtPl) {
          ...
          console.log(
            `${gwDiagTs()} old plugin entry keys:`,
            Object.keys(currentObj.plugins?.entries ?? {})
              .sort()
              .join(','),
          );
          console.log(
            `${gwDiagTs()} new plugin entry keys:`,
            Object.keys(nextObj.plugins?.entries ?? {})
              .sort()
              .join(','),
          );
        } else {
        ...
        console.log(
          `${gwDiagTs()} top-level changed keys:`,
          changedTopLevelKeys.join(',') || '(none)',
        );
      } catch {
        /* ignore parse errors in diag */
      }
      try {
    ...
    return {
      ...
      ...(changedTopLevelKeys.includes('mcp')
  ...
  collectSecretEnvVars(): Record<string, string> {
    ...
    const D = gwDiagTs;
    const keysSummary = Object.keys(env)
      .sort()
      .map(k => {
        const v = env[k];
    ...
    console.log(
      `${D()} collectSecretEnvVars: ${Object.keys(env).length} keys: ${keysSummary.join(', ')}`,
    );
   ...
   * `.clobbered.<timestamp>` snapshot. Because the app writes openclaw.json
  ...
  private stampConfigMeta(config: Record<string, unknown>): Record<string, unknown> {
    ...
    try {
      version = this.engineManager.getStatus().version || this.engineManager.getDesiredVersion();
    } catch {
  ...
  private buildBrowserConfig(browserWebAccess: BrowserWebAccessConfig): Record<string, unknown> {
    ...
    const ssrfPolicy =
      browserWebAccess.networkMode === BrowserNetworkMode.Strict
        ? {
  ...
  /**
   * Ensures exec-approvals.json under the app-managed openclaw home has
  ...
  /**
   * Resolve the app SKILLs installation directory for OpenClaw's
   * `skills.load.extraDirs` configuration.
   *
   * Cross-platform paths (via Electron app.getPath('userData')):
   *   macOS:   ~/Library/Application Support/宇智汇和 AI 助手/SKILLs
   *   Windows: %APPDATA%/宇智汇和 AI 助手/SKILLs
   *   Linux:   ~/.config/宇智汇和 AI 助手/SKILLs
   */
  ...
  /**
   * Build per-skill `enabled` overrides from the app SkillManager state,
   * so that skills disabled in the app UI are also hidden from OpenClaw.
   */
  ...
  private syncAgentsMd(workspaceDir: string, coworkConfig: CoworkConfig): string | undefined {
    const MARKER = `<!-- ${APP_DISPLAY_NAME} managed: do not edit below this line -->`;
    const LEGACY_MARKERS = ['<!-- LobsterAI managed: do not edit below this line -->'];
    ...
    try {
      ...
      // Extract user content (everything before the current or legacy managed marker).
      const markerIdx =
  ...
  private buildAgentsList(
    ...
    const effectiveSkillIds =
      effectiveSettings?.sources.skillIds !== SettingScope.Default
  ...
  private buildBindings(): { bindings?: Array<Record<string, unknown>> } {
    ...
    const multiInstanceChannels: Record<
      string,
      {
        channel: string;
        getInstances: () => Array<{
    ...
    > = {
      dingtalk: {
        ...
        getInstances: () => this.getDingTalkInstances(),
      },
    ...
    const singleInstanceChannels: Array<{
      ...
      {
        getter: () => this.getNeteaseBeeChanConfig(),
        channel: 'netease-bee',
        platform: 'netease-bee',
      },
