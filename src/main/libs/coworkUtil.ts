...
function resolveWindowsGitBashPath(): string | null {
  ...
  // 2. Bundled PortableGit (preferred default in the app package)
...
function ensureWindowsBashUtf8InitScript(): string | null {
  try {
    ...
    const content = [
      '#!/usr/bin/env bash',
      `# Auto-generated by ${APP_DISPLAY_NAME} - switch Windows console code page to UTF-8`,
...
function applyPackagedEnvOverrides(env: Record<string, string | undefined>): void {
  ...
  const nodePaths = [
    ...
    join(resourcesPath, RuntimeBrand.BundleDirName, 'node_modules'),
    join(resourcesPath, RuntimeBrand.LegacyBundleDirName, 'node_modules'),
