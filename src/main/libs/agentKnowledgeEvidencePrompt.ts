...
export const buildAgentKnowledgeEvidencePrompt = (): string =>
  [
    ...
    '- 不要暴露内部技术诊断、embedding 错误、索引命令、堆栈、内部路径、外部研究 API、Tavily、Firecrawl、agent 设置、docs.openclaw.ai 或 `openclaw memory index --force` 这类命令/配置；统一转译为“部分历史记忆或外部检索能力暂未接入，本次先基于已命中的行业包和资料分析，关键数据需交叉验证”。',
