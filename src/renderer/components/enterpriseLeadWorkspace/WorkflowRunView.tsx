...
export const WorkflowRunView: React.FC<WorkflowRunViewProps> = ({ workspace, onOpenCowork }) => {
  ...
  return (
    <div className="min-h-0 flex-1 overflow-y-auto bg-background px-5 py-5 lg:px-7">
      <header className="flex flex-wrap items-start justify-between gap-4"><h1 className="text-xl font-semibold text-foreground">{i18nService.t('enterpriseLeadWorkflowTitle')}</h1><button type="button" onClick={onOpenCowork} className="rounded-md border border-border px-3 py-2 text-xs font-semibold text-foreground">{i18nService.t('enterpriseLeadWorkflowOpenCowork')}</button></header>
